param(
    ...
    [string] $Model = "",
    [string] $RunId = "",
    [string] $OutputRoot = "experiment-runs\tpch-evaluation",
    [int] $WorkloadLimit = 0,
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    ...
    $RunId = "tpch-sf$ScaleFactor-$CandidateSource-$stamp"
}
...
$env:TPCH_QUERIES_PATH = "/app/tpch/queries-parameterized"
$env:TPCH_PARAMETERS_PATH = "/app/tpch/parameters-parameterized"
...
switch ($CandidateSource) {
    "baseline" {
        $env:TPCH_WORKLOAD_MANIFEST_FILE = "/app/tpch/tpch-parameterized-baseline-corpus.json"
        $env:TPCH_ENABLE_RULES = "false"
        $env:TPCH_ENABLE_LLM = "false"
        $env:TPCH_MAX_LLM_CANDIDATES = "0"
    }
    "rule" {
        $env:TPCH_WORKLOAD_MANIFEST_FILE = "/app/tpch/tpch-parameterized-rule-corpus.json"
        $env:TPCH_ENABLE_RULES = "true"
        $env:TPCH_ENABLE_LLM = "false"
        $env:TPCH_MAX_LLM_CANDIDATES = "0"
    }
    "llm" {
        $env:TPCH_WORKLOAD_MANIFEST_FILE = "/app/tpch/tpch-parameterized-local-llm-corpus.json"
        $env:TPCH_ENABLE_RULES = "false"
    ...
    "mixed" {
        $env:TPCH_WORKLOAD_MANIFEST_FILE = "/app/tpch/tpch-parameterized-mixed-corpus.json"
        $env:TPCH_ENABLE_RULES = "true"
        $env:TPCH_ENABLE_LLM = "true"
        $env:TPCH_MAX_LLM_CANDIDATES = "1"
        $env:DEFAULT_MODEL = $Model
...
$runWorkloadEntries = if ($WorkloadLimit -gt 0) { $WorkloadLimit } else { 22 }
...
try {
    ...
    if ($WorkloadLimit -gt 0 -or $SearchParameterLimit -gt 0 -or $HeldOutParameterLimit -gt 0) {
        $scaleRoot = (Resolve-Path -LiteralPath ".\benchmark-data\tpch\sf$ScaleFactor").Path
        ...
        New-ReducedWorkloadFiles `
            -ManifestPath (Join-Path $scaleRoot $manifestFileName) `
            -OutputManifestPath (Join-Path $scaleRoot $limitedManifestFileName) `
            -ParameterDirectory (Join-Path $scaleRoot "parameters-parameterized") `
            -Suffix $RunId `
            -WorkloadLimit $WorkloadLimit `
            -SearchParameterLimit $SearchParameterLimit `
            -HeldOutParameterLimit $HeldOutParameterLimit
        $env:TPCH_WORKLOAD_MANIFEST_FILE = "/app/tpch/$limitedManifestFileName"
    }
    ...
    if ($Monitoring) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TPC-H monitoring export failed with exit code $LASTEXITCODE."
        }
    ...
    if (-not $SkipAnalysis) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TPC-H post-hoc analysis failed with exit code $LASTEXITCODE."
        }
    }

    Write-Host "TPC-H evaluation exports written to $exportDirectory"
}
finally {
